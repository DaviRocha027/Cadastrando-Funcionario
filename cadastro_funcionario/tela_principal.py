...
class Ui_mainWindow(object):
    ...
    def Salvar(self):
        ...
        cargo = self.lineEdit_2.text()
        dtadmissao = self.lineEdit_3.text()
        salario = self.lineEdit_4.text()
        if self.escolha_homem.isChecked():
            genero = "Masculino"
        elif self.escolha_mulher.isChecked():
            genero = "Feminino"
        else:
            genero = "Nao informado"
        resultado = f"Nome:{nome}\nCargo:{cargo}\nData de Admissao:{dtadmissao}\nSalario: R$ {salario}\nGenero:{genero}"
        print(resultado)
